...
public interface ConsultationMedicalRepository extends JpaRepository<ConsultationMedical, Long> {
    ...
    @Query("SELECT DISTINCT p from ConsultationMedical p where p.circuitPatient.patientId=:code and p.actif=1 ORDER BY p.id DESC")
    List<ConsultationMedical> findConsultationMedicalByPatientId(@Param("code") Long code);
    ...
    @Query("SELECT COUNT(c) from ConsultationMedical c where c.circuitPatient.patientId=:code and c.actif=1")
    long countActiveConsultationMedicalByPatient(@Param("code") Long code);
