...
public interface AvisSpecialisteRepository extends JpaRepository<AvisSpecialiste, Long> {
    ...
    @Query("SELECT DISTINCT p from AvisSpecialiste p where p.circuitPatient.patientId=:code and p.actif=1 ORDER BY p.id DESC")
    List<AvisSpecialiste> findAvisSpecialisteByPatientId(@Param("code") Long code);
    ...
    @Query("SELECT DISTINCT p from AvisSpecialiste p where p.circuitPatient.id=:code and p.actif=1 ORDER BY p.id DESC LIMIT 3")
    List<AvisSpecialiste> findTop3ByOrderByIdDesc(@Param("code") Long code);
